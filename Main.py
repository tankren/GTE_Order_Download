...
class Worker(QThread):
  ...
  def send_mail(self):
    ...
    if not file_list:
        email_content = f"Dear all,\n\tGTE {self.ym} 没有新的订单, 请知悉! \n\nVHCN ICO"
    else:
        email_content = f"Dear all,\n\t附件为GTE {self.ym} 订单, 请查收! \n\nVHCN ICO"
    #insert content    
        ...
        for filenm in xml.iter('{*}FileNm'):
            try:
                ...
                for ancestor in filenm.iterancestors('{*}IPO0710DTO'):
                    ...
                    dld_path = f'http://192.168.10.33/GTESGFile/Instruct/3334-A/{self.user[0:5]}/{filenm.text}'
                    #e.g.: http://192.168.10.33/GTESGFile/Instruct/3334-A/20220908/M23334A2022090805ZL.zip

                    message = f'下载Zip文件 {filenm.text}'
                    self.sinOut.emit(message)
...
class MyWidget(QWidget):
    ...
    def get_ordfrom_ordtill(self):
        ...
        if not self.ordfrom == '' and not self.ordtill == '':
            ...
            self.setWindowTitle(f'{self.title} - 订单日期范围: {self.ordfrom}到{self.ordtill}')
        ...
        if user == '' or pwd == '':
            ...
            if not rec == '':
                ...
                if(confirm == QMessageBox.Yes):
                    ...
                    self.thread.getdata(year, till, user, pwd, rec, chk_dld, '0', timer, chk_workday) #订单开始日期，订单结束日期，用户名，密码，收件人，包含已下载，计划执行=0，定时时间，仅工作日）
                    self.thread.start()
            else:
    ...
    def execute_once(self):
        ...
        if user == '' or pwd == '':
            ...
            if not rec == '':
                ...
                self.thread.getdata(self.ordfrom, self.ordtill, user, pwd, rec, chk_dld, '1', '', '') #订单开始日期，订单结束日期，用户名，密码，收件人，包含已下载，单次执行=1，定时时间，仅工作日）
                self.thread.start()
            else:
                self.msgbox('error', '请输入邮箱地址!! ')
